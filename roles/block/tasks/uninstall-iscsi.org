...
- name:  Remove Pool and Disk
  shell: |
    ...
    cd /iscsi-targets/{{ item.wwn }}/hosts/{{ item.init_wwn }}
    disk remove {{ item.pg_name }}/{{ item.image }}
    exit
    EOF
...
- name: Delete Disk
  shell: |
    cat << EOF | {{ podman_iscsi_gw_cmd }} gwcli
    cd /iscsi-targets/{{ item.wwn }}/disks
    delete {{ item.pg_name }}/{{ item.image }}
    exit
    EOF
...
- name: Delete iSCSI Gateways
  shell: |
    ...
    delete rk9-node01.jtest.weka.io confirm=true
    delete rk9-node02.jtest.weka.io confirm=true
    delete rk9-node03.jtest.weka.io confirm=true
    exit
    EOF
...
- name: Remove iSCSI Gateway
  shell: |
    ceph dashboard iscsi-gateway-rm {{ item }}.jtest.weka.io
